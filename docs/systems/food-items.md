...
- `edible: false` marks an intermediate item. `seafood_whole` is a CustomFishing catch or a plain vanilla fish: it keeps the source material and model, stores size and cut class, and is not a finished food. CustomFishing also stores the loot id. Cutting it produces `seafood_fish_filet` (nutrition 6), `seafood_jellyfish` (nutrition 5), or `seafood_octopus` (nutrition 7). Those portions are edible. Their food is stored per piece. Their models include cooked, burnt, and boiled states, and rotten still wins over those states. Each portion has a pot option (`tag: 3`, 15 seconds, burn at 30). Default edible is true.
